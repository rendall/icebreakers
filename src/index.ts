...
const removeListMarkdown = (line: string) => line.slice(line.indexOf("*") + 1).trim();
...
const createUI = (
  questions: { question: string; credit: { name: string; href: string } }[]
	...
	const displayQuestion = (question: Question) => {
		...
		const creditLink = document.querySelector("#credit-link") as HTMLAnchorElement;
...
const onLoad = () => loadQuestion().then(parseQuestions).then(shuffleQuestions).then(createUI);

// parse QUESTIONS.md
// array of { question, credit }
// create UI
// question display
// reload button
// credit field
